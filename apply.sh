...
original_author="omagebright"
original_name="stingallo_web"
original_urlname="stingallo_web"
original_description="Awesome stingallo_web created by omagebright" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/stingallo_web "${name}"
...
then
    ...
    echo "then run 'stingallo_web' or 'python -m stingallo_web'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'stingallo_web' or 'python -m stingallo_web' or 'poetry run stingallo_web'"
fi
